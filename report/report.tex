...
\begin{figure}[H]
  ...
  \fbox{\parbox{0.85\textwidth}{\centering\vspace{3.2cm}
        \textit{Placeholder: Bézier/B-spline curve editing interface showing curve
                 visualization with draggable control point handles, control point
                 coordinates editor panel, and degree/knot configuration controls.}
        \vspace{3.2cm}}}
...
\begin{figure}[H]
  ...
  \fbox{\parbox{0.85\textwidth}{\centering\vspace{2.8cm}
        \textit{Placeholder: Surface Generation panel in GUI showing source
                 mode selector (Control Net, Loft, Extrude), conditional inputs
                 for parameters (rows/columns, curves list, axis, height, layers),
                 and Generate Surface button.}
        \vspace{2.8cm}}}
  \caption{GUI panel for surface generation control. Users select surface
           creation method and adjust parameters (e.g., rows/columns for grid,
           extrusion height, number of layers). The Generate Surface button
...
\begin{figure}[H]
  \centering
  \fbox{\parbox{0.85\textwidth}{\centering\vspace{3.2cm}
        \textit{Placeholder: Toolpath visualization in PyVista viewport showing
                 the NURBS surface with overlaid zigzag toolpath passes rendered
                 as colored polylines (scanlines), CL points as markers, and
                 surface normals as vectors at selected points.}
        \vspace{3.2cm}}}
  \caption{Rendered toolpath showing the zigzag scanning pattern overlaid on
           the NURBS surface. Different pass colors distinguish forward/reverse
           directions. Cutter location (CL) points are displayed above the
           surface, offset along surface normals by tool radius.}
...
\begin{figure}[H]
  \centering
  \fbox{\parbox{0.85\textwidth}{\centering\vspace{3.0cm}
        \textit{Placeholder: G-Code Export panel in GUI showing feed rate,
                 plunge rate, safe Z settings, spindle RPM controls, and the
                 Export G-code button. Also shows file save dialog or exported
                 .nc file preview in text editor.}
        \vspace{3.0cm}}}
  \caption{G-Code export interface showing CNC machine configuration parameters
           ...
           button for generating ISO 6983-compliant machine code files.}
